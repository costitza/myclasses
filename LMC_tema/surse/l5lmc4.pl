...
/* verificarea pastrarii relatiilor de catre functia F:A->B, mai precis a faptului ca F duce relatia binara R<=AxA in relatia binara S<=BxB: */

pastrrel(F,R,S) :- not((member((X,Y),R),
	member((X,FX),F),member((Y,FY),F), not(member((FX,FY),S)))).

/* generarea functiilor izotone (i.e. crescatoare) F de la posetul (P,OrdP) la posetul (Q,OrdQ), prin selectarea dintre functiile F:P->Q generate cu predicatul functie pe cele care satisfac predicatul anterior pentru R:=OrdP si S:=OrdQ: */

fctcresc(F,P,OrdP,Q,OrdQ) :- functie(F,P,Q), pastrrel(F,OrdP,OrdQ).

/* generarea functiilor strict izotone (i.e. strict crescatoare) F de la posetul (P,OrdP) la posetul (Q,OrdQ), prin selectarea dintre functiile F:P->Q generate cu predicatul functie pe cele care satisfac predicatul pastrrel pentru R := ordinea stricta a lui (P,OrdP) si S := ordinea stricta a lui (Q,OrdQ): */

fctstrcresc(F,P,OrdP,Q,OrdQ) :- functie(F,P,Q),
	ordstrdinord(OrdP,OrdStrP), ordstrdinord(OrdQ,OrdStrQ),
	pastrrel(F,OrdStrP,OrdStrQ).
...
/* lista LF a functiilor strict crescatoare de la posetul (P,OrdP) la posetul (Q,OrdQ): */

functiistrcresc(P,OrdP,Q,OrdQ,LF) :- 
...
% lantul cu (exact) 4 elemente:

l4(L4,OrdL4) :- L4=[0,m,p,1], orddinsucc([(0,m),(m,p),(p,1)],L4,OrdL4).
...
?- posetA(P,OrdP), l4(Q,OrdQ), functiicresc(P,OrdP,Q,OrdQ,LFcresc), functiistrcresc(P,OrdP,Q,OrdQ,LFstrcresc), afislistafct(LFcresc,P), length(LFcresc,NrFctCresc), afislistafct(LFstrcresc,P), length(LFstrcresc,NrFctStrCresc).
?- posetA(P,OrdP), posetB(Q,OrdQ), functiicresc(P,OrdP,Q,OrdQ,LFcresc), functiistrcresc(P,OrdP,Q,OrdQ,LFstrcresc), afislistafct(LFcresc,P), length(LFcresc,NrFctCresc), afislistafct(LFstrcresc,P), length(LFstrcresc,NrFctStrCresc).
...
izomposeturi(F,P,OrdP,Q,OrdQ) :- bijectie(F,P,Q), pastrrel(F,OrdP,OrdQ),
	invrel(F,FlaMinus1), pastrrel(FlaMinus1,OrdQ,OrdP).
